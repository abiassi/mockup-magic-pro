...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  // Dynamic imports to surface which module fails
  const errors: string[] = [];
  ...
  } catch (e: any) { errors.push(`McpServer: ${e.code} ${e.message}`); }

  let WebTransport: any;
  try {
    const mod = await import('@modelcontextprotocol/sdk/server/webStandardStreamableHttp.js');
    WebTransport = mod.WebStandardStreamableHTTPServerTransport;
  } catch (e: any) { errors.push(`WebTransport: ${e.code} ${e.message}`); }

  let z: any;
  try {
    const mod = await import('zod');
    z = mod.z;
  } catch (e: any) { errors.push(`zod: ${e.code} ${e.message}`); }

  let neon: any;
  try {
    const mod = await import('@neondatabase/serverless');
    neon = mod.neon;
  } catch (e: any) { errors.push(`neon: ${e.code} ${e.message}`); }

  let serverFns: any;
  try {
    serverFns = await import('./_lib/geminiServerService');
  } catch (e: any) { errors.push(`geminiServerService: ${e.code} ${e.message}`); }

  if (errors.length > 0) {
    return res.status(500).json({ importErrors: errors });
  }

  // If all imports succeeded, handle the MCP request
  try {
    const { serverGenerateMockup, serverGenerateComposite, serverAnalyzeArtwork } = serverFns;
    const mcp = createMcpServer(McpServer, z, neon, serverGenerateMockup, serverGenerateComposite, serverAnalyzeArtwork);
    const transport = new WebTransport({ sessionIdGenerator: undefined });
...
function createMcpServer(
  McpServer: any, z: any, neon: any,
  serverGenerateMockup: any, serverGenerateComposite: any, serverAnalyzeArtwork: any
) {
  ...
  mcp.tool(
    ...
    async (args: any) => {
      ...
      for (const imageUrl of images) {
        ...
        await sql`INSERT INTO mockup_results (id, image_url, prompt, created_at, aspect_ratio, camera_angle) VALUES (${id}, ${imageUrl}, ${args.prompt}, ${createdAt}, ${settings.aspectRatio}, ${args.camera_angle ?? null}) ON CONFLICT (id) DO NOTHING`;
  ...
  mcp.tool(
    ...
    async (args: any) => {
      const images = await serverGenerateComposite(args.base_image_base64, args.artwork_base64, args.instructions, args.aspect_ratio ?? '3:4', '2K');
      const imageUrl = images[0];
      const sql = getSql();
      const id = crypto.randomUUID();
      const createdAt = Date.now();
      await sql`INSERT INTO mockup_results (id, image_url, prompt, created_at, aspect_ratio, variant_type, composite_base_url, composite_artwork_url) VALUES (${id}, ${imageUrl}, ${args.instructions}, ${createdAt}, ${args.aspect_ratio ?? null}, ${'composite'}, ${args.base_image_base64.slice(0, 100)}, ${args.artwork_base64.slice(0, 100)}) ON CONFLICT (id) DO NOTHING`;
  ...
  mcp.tool(
    ...
    'Analyze artwork and return prompt suggestions for mockup generation.',
    {
      ...
      vibe: z.enum(['Industrial & Raw', 'Modern & Minimalist', 'Cozy & Bohemian', 'Luxury & High-end', 'Public & Street', 'Surprise Me']).optional().describe('Desired vibe/style'),
    },
    async (args: any) => {
      const suggestions = await serverAnalyzeArtwork(args.artwork_base64, args.vibe);
  ...
  mcp.tool(
    ...
    async (args: any) => {
      ...
      const rows = await sql`SELECT id, prompt, created_at AS "createdAt", is_high_res AS "isHighRes", is_contact_sheet AS "isContactSheet", camera_angle AS "cameraAngle", variant_type AS "variantType", aspect_ratio AS "aspectRatio" FROM mockup_results ORDER BY created_at DESC LIMIT ${limit} OFFSET ${offset}`;
  ...
  mcp.tool(
    ...
    async (args: any) => {
      ...
      if (rows.length === 0) return { content: [{ type: 'text', text: JSON.stringify({ error: 'Mockup not found' }) }], isError: true };
  ...
  mcp.tool(
    ...
    async (args: any) => {
      ...
      const input = isClarity
        ? { image: args.image_base64, scale_factor: 2, creativity: 0.15, resemblance: 0.9, prompt: 'high resolution, sharp details, professional photography', negative_prompt: 'blurry, artifacts, noise' }
